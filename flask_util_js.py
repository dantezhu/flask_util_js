...
var flask_util = function() {
  ...
  function url_for(endpoint, params) {
    if (!params) {params = {}}
    if (!rule_map[endpoint]) {throw('endpoint does not exist: ' + endpoint)}
    ...
    var query_string = '';
    for (var k in params) {
      if (used_params.hasOwnProperty(k)) {continue}
      var v = params[k];
      if (query_string.length > 0) {query_string += '&'}
      query_string += encodeURIComponent(k) + '=' + encodeURIComponent(v);
    }
    var url = path;
    if (query_string.length > 0) {url += '?' + query_string}
    return url;
  }
  return {url_for: url_for, rule_map: rule_map}
}();
{%- endautoescape -%}
'''
